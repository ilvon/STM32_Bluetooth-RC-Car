...
{	
	...
	if(usPAGE > 255){
		LCD_Write_Cmd(0x2B);
		LCD_Write_Data(1);
		LCD_Write_Data(usPAGE-256);
		LCD_Write_Data(1);
		LCD_Write_Data(usPAGE-256);
	}else{
		LCD_Write_Cmd(0x2B);
		LCD_Write_Data(0);
		LCD_Write_Data(usPAGE);
		LCD_Write_Data(0);
		LCD_Write_Data(usPAGE);
	}
	LCD_Write_Cmd(0x2C);
...
void LCD_QRcode(){
	LCD_DrawString(20,10,"Payment:");
	for(int i=0; i<50; i++){
		for(int j=0; j<50; j++){
			if(qrdata[i][j] == '1'){LCD_DrawDot(75+j,135+i,WHITE);}
			else{LCD_DrawDot(75+j,135+i,BLACK);}
		}
	}
}
